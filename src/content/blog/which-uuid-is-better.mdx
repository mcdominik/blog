...
> ⚠️ Choosing the wrong one can hurt your database performance at scale.!

## What is a [UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier)?

**Universally Unique Identifier** (UUID) is a 128-bit number used (as the name suggests) to uniquely identify objects in distributed systems.
...
Both are great, but **UUID v7** can offer better performance. Its time-sorted nature is a game-changer for high-performance database inserts (keeping the [B-tree](https://en.wikipedia.org/wiki/B-tree) index happy).
...
However, most relational databases (like Postgres) have a native `UUID` data type. They store these as 128-bit binary data under the hood, which is much more efficient than storing a NanoID as a `VARCHAR` string.
...
- **UUID v7** shines at scale; if your table is less than 10M rows, UUID v4 is just fine.
